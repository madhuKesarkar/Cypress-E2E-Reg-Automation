...
class SubsidiesPage {
  ...
  goToPaymentsTab() {
    this.paymentsNav.should('be.visible').click();
    cy.url({ timeout: Cypress.config('responseTimeout') }).should('include', '/transactions/payments');
    return this;
  }
  ...
  goToStudentsTab() {
    this.studentsNav
      ...
      .and('not.have.attr', 'aria-disabled', 'true')
      .click();
    cy.url({ timeout: Cypress.config('responseTimeout') }).should('include', '/students');
    return this;
  }
  ...
  goToUpcomingInvoices() {
    this.upcomingInvoicesLink.should('be.visible').click();
    cy.url({ timeout: Cypress.config('responseTimeout') }).should('include', '/upcoming');
    return this;
  }
  ...
  goToAgencyInfo() {
    this.agencyInfoLink.should('be.visible').click();
    cy.url({ timeout: Cypress.config('responseTimeout') }).should('include', '/info');
    return this;
  }
  ...
  assertPageHeading() {
    this.pageHeading.should('be.visible');
    cy.url({ timeout: Cypress.config('responseTimeout') }).should('include', '/billing/subsidies/all-agencies');
    return this;
  }
  ...
  assertAgencyCreated(agencyName) {
    cy.url({ timeout: Cypress.config('responseTimeout') })
      .should('include', '/billing/subsidies/')
      .and('include', '/transactions/invoices');
    cy.contains('h1', agencyName, { timeout: 30000 }).should('be.visible');
    return this;
